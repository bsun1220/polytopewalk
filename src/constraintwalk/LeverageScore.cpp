...
VectorXd LeverageScore::generate(const SparseMatrixXd& A, const SparseMatrixXd& W, const VectorXd& x){

    VectorXd x_prime = x;
    x_prime.array() += 0.000001;
    SparseMatrixXd S_inv = SparseMatrixXd(x_prime.cwiseInverse().asDiagonal());
    SparseMatrixXd G_inv = S_inv * W * S_inv;
    ...
    SparseMatrixXd res (AG_inv.cols(), AG_inv.cols());
    for(int i = 0; i < AG_inv.cols(); i++){
        double val = AG_inv.col(i).dot(P.col(i));
        res.coeffRef(i, i) = val;
    }
    res = G_inv - res;
    res = S_inv * res * S_inv;
    return res.diagonal();
